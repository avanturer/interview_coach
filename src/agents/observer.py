"""Observer agent - analyzes answers behind the scenes."""
...
class ObserverAgent(BaseAgent):
    """Analyzes candidate answers and provides instructions to Interviewer."""
    ...
    def _process_response(self, state: InterviewState, response: str) -> dict[str, Any]:
        ...
        # Override stop detection with explicit user intent check
        ...
        # Format detailed internal thoughts showing multi-agent collaboration
    ...
    def _format_detailed_thoughts(self, analysis: ObserverAnalysis, state: InterviewState) -> str:
        """Format internal thoughts for multi-agent collaboration."""
    ...
    def _parse_analysis(self, response: str) -> ObserverAnalysis:
        ...
        try:
            ...
            # Fallback - continue interview when parsing fails
            return ObserverAnalysis(
                wants_to_end_interview=False,  # Don't stop on parse error!
                wants_to_skip=False,
                is_valid_answer=True,
                answer_quality=5,
                instruction_to_interviewer="Продолжай интервью, задай следующий технический вопрос.",
                thoughts=f"Parse error: {e}. Continuing interview.",
            )
    
    def _check_user_stop_intent(self, user_message: str) -> bool:
        """Check if user explicitly wants to stop the interview."""
